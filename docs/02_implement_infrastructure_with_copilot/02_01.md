...
1. From Chat with Copilot enter the following prompt to create an issue:
    ...
    Foundry, I want to use swedencentral because they are available there. Everything
...
application (dev environment) into a single resource group in swedencentral. Use
...
swedencentral as these models are available there.

## Requirements
- All resources provisioned into a single resource group (e.g. `rg-zavastore-dev-swedencentral`) in swedencentral
...
- Microsoft Foundry resource provisioned in swedencentral
...
## Checklist
- [ ] Confirm region and subscription quotas for Microsoft Foundry in swedencentral
